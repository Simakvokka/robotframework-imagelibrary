...
from ImageLibrary.button import Button, StatedButton, get_button_info, STATE_DISABLED, STATES_ACTIVE
...
class DynamicButton(StatedButton):
    ''' Dynamic button can be on screen. And can not be on screen. And can move on screen.
        So before press it - slotbot will find it's image on screen and press actual coordinates.
        Button will be pressed even in disabled state, but warning will be writen to log.
...
class DynamicButtonList(Button):
    ...
    @utils.add_error_info
    def wait_for_button_state(self, index, state, timeout, step):
        return self._get_button_by_index(index).wait_for_button_state(None, state, timeout, step)

    @utils.add_error_info
    def wait_for_activate_and_press(self, index, timeout, step):
